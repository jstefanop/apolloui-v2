...
const PoolSettings = () => {
  ...
  // Backup/failover pool (Apollo III only) is intentionally not rendered yet: the
  // feature is blocked on the Apollo III firmware + confirmed miner CLI flags, and
  // the backend does not emit backup-pool config either. Re-add it alongside that
  // work (state + handlers, DeviceConfigContext hasInternalMiner/isHybrid flags).
