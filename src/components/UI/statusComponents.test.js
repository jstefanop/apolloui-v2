...
// The drive filling up is a fact about the hardware, not about the service, so
// the notice rides above whatever the service state is — including "running",
// which is exactly the state in which it is about to stop.
describe('NodeStatus with the drive running out of space', () => {
  const online = { node: { status: 'online', requestedStatus: 'online' } };
  const GB = 1024 * 1024 * 1024;

  it('warns above a running node, with the figure', () => {
    withStorage({ low: true, free: 5 * GB });
    renderWithProviders(<NodeStatus serviceStatus={online} loading={false} />);
  ...
  it('warns above a stopped node too — that is usually why it stopped', () => {
    withStorage({ low: true, free: 0.2 * GB });
    renderWithProviders(
      <NodeStatus
        serviceStatus={{ node: { status: 'offline', requestedStatus: 'online' } }}
        loading={false}
      />
    );
  ...
  it('says nothing while there is room', () => {
    withStorage({ low: false, free: 400 * GB });
    renderWithProviders(<NodeStatus serviceStatus={online} loading={false} />);

    expect(screen.queryByText(/running out of space/i)).not.toBeInTheDocument();
  });

  it('does not mistake an unmeasured drive for a full one', () => {
    withStorage({ low: false, free: null });
    renderWithProviders(<NodeStatus serviceStatus={online} loading={false} />);
